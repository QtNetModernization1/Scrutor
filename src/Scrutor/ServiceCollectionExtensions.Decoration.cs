...
/// using the specified type <typeparamref name="TDecorator"/>.
    ...
    // Overload to handle IEnumerable<ServiceDescriptor> explicitly
    public static bool TryDecorate<TService, TDecorator>(this IEnumerable<ServiceDescriptor> services)
        where TDecorator : TService
    {
        Preconditions.NotNull(services, nameof(services));

        return (services as IServiceCollection ?? new ServiceCollection(services)).TryDecorate(typeof(TService), typeof(TDecorator));
    }
